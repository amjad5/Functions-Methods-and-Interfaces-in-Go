...
The user can issue a request to find out one of three things about an animal: 1) the food that it eats, 2) its method of locomotion, and 3) the sound it makes when it speaks. The following table contains the three animals and their associated data which should be hard-coded into your program.
...
func getInformation(animal *Animal, info string ){
	switch{
		case info == "eat":
			animal.Eat()
		case info == "move":
...
func main (){
	for {
		...
		fmt.Println("")
		fmt.Println("Please type animal and information separated by space or x to exit")
		fmt.Println("animals: 'cow', 'bird', 'snake' and information 'eat', 'move' or 'speak' ")
		fmt.Println("example: cow eat")
		_, err := fmt.Scanf("%s %s",&animal, &info)
		if err != nil {
			if animal != "x"{
				fmt.Println("Plase enter animal name and information requested correctly!")
			}
			break
		}
		switch{
